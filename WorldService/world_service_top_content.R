setwd("/data/users/VickyBanks/sounds_international/world_service")
#console_messages_filename <- file(paste(paste("console_messages_world_service_top_sounds_content",".csv"), sep =""), open = "a")
#sink(console_messages_filename, append=TRUE, type=c("message"))


print(Sys.time())
################## 0. Set up AWS creds (only works from server) ########################
# New dependency
library("aws.secrets")
require("RJDBC")
require("jsonlite")
...
# Function to get credentials from AWS and return a working connection to redshift (only works from server)
# @param secret_location The location in AWS where the secret is saved
# @param driver_location The local location where the driver file is
# @return an open connection to redshift
...
# 12. top 10 countries by playback time - every week
# 
conn <- get_redshift_connection() #Because there are lots of queries here and the connecion times out, re-do the connection
...
send.mail(from = "vicky.banks@bbc.co.uk",
          to = c("katherine.campbell@bbc.co.uk", "anna.doble@bbc.co.uk"),
